...
def getParsedData():
    ...
    publications = get_publications(document)
...
def get_skills(document):
    skills = []
    skills_summary_flag = False
    skills_summary = ''
    exact_matches = ['SKILLS','TECHNICAL SKILLS']
    contains = []
    for line in document:
        if line[0].isupper() and (any(line.upper().strip() == word for word in exact_matches) or any(word in line.upper() for word in contains) ) and (len(line.strip().split(' '))<6 or (line.find(':')!=-1 and len(line[:line.find(':')].strip().split(' '))<6)):
          skills_summary_flag = True 
          #skills.append([line])
        elif skills_summary_flag:
          if line.strip()[0].isupper() and any(word.title() in line or word in line for word in stopwords) and len(line.strip().split(' '))<6 and (line.find(':')==-1 or (any(word in line[:line.find(':')].upper() for word in stopwords) and line.strip() == line[:line.find(':')+1].strip())): 
            skills_summary_flag = False
          else:
            skills_summary += line
...
def get_publications(document):
    publications = []
    publication_summary_flag = False
    publication_summary = ''
    exact_matches = ['PUBLICATIONS', 'RESEARCH PAPERS']
    contains = []
    for line in document:
        if line[0].isupper() and (any(line.upper().strip() == word for word in exact_matches) or any(word in line.upper() for word in contains) ) and (len(line.strip().split(' '))<6 or (line.find(':')!=-1 and len(line[:line.find(':')].strip().split(' '))<6)):
          publication_summary_flag = True 
          #publications.append([line])
        elif publication_summary_flag:
          if line.strip()[0].isupper() and any(word.title() in line or word in line for word in stopwords) and len(line.strip().split(' '))<6 and (line.find(':')==-1 or (any(word in line[:line.find(':')].upper() for word in stopwords) and line.strip() == line[:line.find(':')+1].strip())): 
            publication_summary_flag = False
          else:
            publication_summary += line
            publications.append(line)
    return(publications)
...
def extractDataJob(jobExperience,finder,company_tagger):
    ...
    skills_labels = ['ENVIRONMENT','SKILLS','TECHNICAL SKILLS','APPLICATIONS','DOMAIN','INDUSTRY','SERVICE','DESCRIPTION','RESPONSIBILITES','SIZE','PERIOD','DURATION','TECHNOLOGY','TOOLS','ACHIEVEMENTS']
